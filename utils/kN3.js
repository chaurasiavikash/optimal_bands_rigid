...
var findU = function(cn, n, s) {
  ...
  let u = s; // Initial guess
  let tolerance = 1e-8; // Tolerance for the solution
  let maxIterations = 1000; // Maximum number of iterations
  let iteration = 0;
  let error = 10 * tolerance; // Initialize error

  // Newton-Raphson method
  while (error > tolerance && iteration < maxIterations) {
    let uNext = u - f(u) / df(u); // Next approximation
    error = Math.abs(uNext - u); // Update error
    u = uNext; // Update u for the next iteration
    iteration++;
  }

  if (iteration == maxIterations) {
    console.log("Solution did not converge");
    return null;
  }
console.log(f(u));
  return u;
...
// first calculating the rotation angle for rigid transformation such that the x coordinate of the midline at s = 0 is  0
  ...
  let an = n*Math.sqrt(n*n-4);
  let bn = (n*n-4 - an)/n/2.0;
  let cn = bn/an;
  ...
  //   s = s0;
  // }
  //s =  s0 + cn  * Math.sin(2*n*s0);
   


  xi = 0*s0-c*t;    // xi at s = 0
...
rx = rx.map((num) => num -cx);
ry = ry.map((num) => num -cy);
rz = rz.map((num) => num -cz);
...
var fourierExpansion = function (n,N, t, hl) {
  ...
  let len24 = Math.sqrt((x2-x4)**2 + (y2-y4)**2 + (z2-z4)**2);
   ...
   i = 1;
  let rlen12 = Math.sqrt((rx[i]-rx[i+1])**2 +  (ry[i]-ry[i+1])**2 + (rz[i]-rz[i+1])**2 );
   
   console.log(rlen12,len13,len14);
   //console.log(rlen12);
